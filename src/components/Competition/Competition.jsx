...
export default function Competition({ title, date, info, place, winner }) {
    ...
    return (
        <div className='competition-wrapper'>
            <h2 className='competition-title'><Text word="" />{title}</h2>
            <div className='competition-date'><Text word="" />Кога: {date}</div>
            <div className='competition-info'><Text word="" />{info}</div>
            <div className='toggle-button' onClick={toggleReadMode}>Read More</div>
            <div className='competition-place'><Text word="" />Къде: {place}</div>
            <div className='competition-winner'><Text word="" />{winner}</div>
        </div>
    );
}
